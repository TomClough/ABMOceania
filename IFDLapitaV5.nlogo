extensions [ gis palette ]
globals [ cities-dataset
          rivers-dataset
          arch-dataset
          countries-dataset
          elevation-dataset
          IslandsOccupied

          best-patches-move

]
breed [ city-labels city-label ]
breed [ arch-labels arch-label ]
breed [ country-labels country-label ]
breed [ country-vertices country-vertex ]
breed [ river-labels river-label ]
breed [ groups group ]
patches-own [ population country-name elevation rain area value pop current-value bio1 bio2 bio3 bio4 bio5 bio6 bio7 bio8 bio9 bio10 bio11 bio12
  bio13 bio14 bio15 bio16 bio17 bio18 bio19 ARCH INAME tstep tstep2
]

groups-own [
  nutrition-need
  nutrition-need-remaining
  archipelago
  islandname
]

to setup
  clear-all
  ; Note that setting the coordinate system here is optional, as
  ; long as all of your datasets use the same coordinate system.
  ;gis:load-coordinate-system (word projection ".prj")
  ; Load all of our datasets
  set arch-dataset gis:load-dataset "Archipelagos.shp"
  set cities-dataset gis:load-dataset "LapitaIslands.shp"
  ;set rivers-dataset gis:load-dataset "data/rivers.shp"
  set countries-dataset gis:load-dataset "FinalOceania.shp"
  ;set elevation-dataset gis:load-dataset "data/world-elevation.asc"
  ; Set the world envelope to the union of all of our dataset's envelopes
  gis:set-world-envelope (gis:envelope-union-of (gis:envelope-of cities-dataset)
                                                (gis:envelope-of countries-dataset))

  gis:apply-coverage countries-dataset "MELEV" elevation
  gis:apply-coverage countries-dataset "AREA" area
  ;gis:apply-coverage countries-dataset "BIO1" bio1
 ; gis:apply-coverage countries-dataset "BIO2" bio2
 ; gis:apply-coverage countries-dataset "BIO3" bio3
  gis:apply-coverage countries-dataset "BIO4" bio4
 ; gis:apply-coverage countries-dataset "BIO5" bio5
 ; gis:apply-coverage countries-dataset "BIO6" bio6
 ; gis:apply-coverage countries-dataset "BIO7" bio7
 ; gis:apply-coverage countries-dataset "BIO8" bio8
 ; gis:apply-coverage countries-dataset "BIO9" bio9
 ; gis:apply-coverage countries-dataset "BIO10" bio10
 ; gis:apply-coverage countries-dataset "BIO11" bio11
  gis:apply-coverage countries-dataset "RAIN" rain
 ; gis:apply-coverage countries-dataset "BIO13" bio13
 ; gis:apply-coverage countries-dataset "BIO14" bio14
 ; gis:apply-coverage countries-dataset "BIO15" bio15
 ; gis:apply-coverage countries-dataset "BIO16" bio16
 ; gis:apply-coverage countries-dataset "BIO17" bio17
 ; gis:apply-coverage countries-dataset "BIO18" bio18
 ; gis:apply-coverage countries-dataset "BIO19" bio19
  gis:apply-coverage countries-dataset "ARCH" ARCH
  gis:apply-coverage countries-dataset "INAME" INAME



;BIO1 = Annual Mean Temperature
;BIO2 = Mean Diurnal Range (Mean of monthly (max temp - min temp))
;BIO3 = Isothermality (BIO2/BIO7) (* 100)
;BIO4 = Temperature Seasonality (standard deviation *100)
;BIO5 = Max Temperature of Warmest Month
;BIO6 = Min Temperature of Coldest Month
;BIO7 = Temperature Annual Range (BIO5-BIO6)
;BIO8 = Mean Temperature of Wettest Quarter
;BIO9 = Mean Temperature of Driest Quarter
;BIO10 = Mean Temperature of Warmest Quarter
;BIO11 = Mean Temperature of Coldest Quarter
;BIO12 = Annual Precipitation
;BIO13 = Precipitation of Wettest Month
;BIO14 = Precipitation of Driest Month
;BIO15 = Precipitation Seasonality (Coefficient of Variation)
;BIO16 = Precipitation of Wettest Quarter
;BIO17 = Precipitation of Driest Quarter
;BIO18 = Precipitation of Warmest Quarter
;BIO19 = Precipitation of Coldest Quarter



  ask patches [
    if(elevation > 0)[
      set elevation (elevation / 2715)
      ;set elevation (((elevation / 2715) * -1) + 1)
      set area (area / 3578122)
      set rain (rain / 4603)
      ;set bio1 (bio1 / 27.5)
      ;set bio2 (bio2 / 8.15)
     ; set bio3 (bio3 / 90.43)
      set bio4 (bio4 / 240.26)
     ; set bio5 (bio5 / 31.18)
     ; set bio6 (bio6 / 24.14)
     ; set bio7 (bio7 / 13.65)
     ; set bio8 (bio8 / 27.63)
     ; set bio9 (bio9 / 27.85)
     ; set bio10 (bio10 / 27.85)
     ; set bio11 (bio11 / 27.37)
    ;  set bio13 (bio13 / 568)
   ;   set bio14 (bio14 / 328.9)
    ;  set bio15 (bio15 / 59.17)
    ;  set bio16 (bio16 / 1575)
    ;  set bio17 (bio17 / 1020.61)
    ;  set bio18 (bio18 / 1242)
    ;  set bio19 (bio19 / 1208.8)
    ]
    set tstep -1
  ]

  set IslandsOccupied []
  set best-patches-move []
  reset-ticks

end

to go
  output-type "Timestep "
  output-type ticks
  output-type ": "
  ;ask patches[
  ;  set value-distance value - distance myself
  ;]

  ask groups [

    ;move 1
    ifelse count groups-on patch-here <= (value) and ticks > 0 [
      set color black
      if random 20 = 1 [
        hatch 1
      ]
      set pop (count groups-on patch-here)
      set current-value (value - pop)
      ;should make method for spawning groups to take care of current value properly, doesnt work for initial spot
      ;if count groups-on patch-here <= value [
      ;  set color blue
      ;]
    ]
    [
     ;set current-value current-value + 1  ;something isnt right here
      if count groups-on patch-here > value [
        move
        set pop (count groups-on patch-here)
        set current-value (value - pop)

      ]
    ]
    ;;

    ;;
    set archipelago [ARCH] of patch-here
    set islandname [INAME] of patch-here
    if not member? islandname IslandsOccupied [
      set IslandsOccupied lput islandname IslandsOccupied
      output-type islandname
      output-type ", "
      set tstep2 ticks
      ask patches with [INAME = [islandname] of myself ] [
        set tstep2 ticks
      ]
    ]

    if (count groups-on patch-here) = 1 and tstep = -1 [
      set tstep ticks
    ;;  set tstep2 ticks
        ;ask patches with
    ;;  if member? [INAME] of patch-here IslandsOccupied [
    ;;   set tstep2 position [INAME] of patch-here IslandsOccupied
    ;;  ]

    ]


  ]


  tick
  output-print " "
  if ticks >= 50 [stop]
end


to move


  if any? patches with [current-value >= 1 ] [

    ifelse not RangeExpan [
  ;if not nobody
      if (count patches with [current-value >= 1 and distance myself < max-trip-distance]) > 0 [
        move-to max-one-of (patches with [current-value >= 1 and distance myself < max-trip-distance] ) [current-value - (distance myself * distance-weighting)]
      ]

    ]
    [
      ;range expansion
      ask groups [
        ;show max-one-of (patches with [current-value >= 1 and distance myself < max-trip-distance] ) [current-value - (distance myself * distance-weighting)]
        ;put all in list then get one with highest current value;
        if not member? max-one-of (patches with [current-value >= 1 and distance myself < max-trip-distance] ) [current-value - (distance myself * distance-weighting)] best-patches-move [
          set best-patches-move lput max-one-of (patches with [current-value >= 1 and distance myself < max-trip-distance] ) [current-value - (distance myself * distance-weighting)] best-patches-move
        ]
      ]
      ;show max-one-of best-patches-move [current-value]
      ;show best-patches-move
      set best-patches-move sort-by [[t1 t2] -> [current-value] of t1 < [current-value] of t2] best-patches-move  ; sort the patches
                                                                                                                  ;show best-patches-move
                                                                                                                  ;show last best-patches-move


      if (count patches with [current-value >= 1 and distance myself < max-trip-distance]) > 0 [
        move-to last best-patches-move
      ]

      set best-patches-move []
    ]

  ]

end


to display-countries
  ask country-labels [ die ]
  gis:set-drawing-color white
  gis:draw arch-dataset 1
  if label-arch
  [ foreach gis:feature-list-of arch-dataset [ vector-feature ->
      let centroid gis:location-of gis:centroid-of vector-feature
       ;centroid will be an empty list if it lies outside the bounds
       ;of the current NetLogo world, as defined by our current GIS
       ;coordinate transformation
      if not empty? centroid
      [ create-country-labels 1
        [

          set xcor item 0 centroid
          set ycor item 1 centroid
          set size 0
          set label-color red
          set label (gis:property-value vector-feature "ARCH")
          set label word label "   "
        ]
      ]
    ]
  ]
end


to display-islands
  ask city-labels [ die ]
  foreach gis:feature-list-of cities-dataset [ vector-feature ->
    ;gis:set-drawing-color red ;(gis:property-value vector-feature "Area") 500000 1000
    ;gis:fill vector-feature 2.0
    if label-islands
    [ ; a feature in a point dataset may have multiple points, so we
      ; have a list of lists of points, which is why we need to use
      ; first twice here
      ;let location gis:location-of (first (first (gis:vertex-lists-of vector-feature)))
      let centroid gis:location-of gis:centroid-of vector-feature
      ; location will be an empty list if the point lies outside the
      ; bounds of the current NetLogo world, as defined by our current
      ; coordinate transformation
      if not empty? centroid ;location
      [ create-city-labels 2
        [ set xcor item 0 centroid ;location
          set ycor item 1 centroid ;location
          set size 0.5
          set shape "circle 2"
          set color red
          set label-color white
          set label gis:property-value vector-feature "IName"
          set label word label "   \n"
        ]
      ]
    ]
  ]
end


to display-value-in-patches


  ask patches
  [
    if(elevation > 0)[
    set value (((area * area-weighting)
        + (rain * rain-weighting)
        + (elevation * elevation-weighting)
;        + (bio1 * bio1-weighting)
 ;       + (bio2 * bio2-weighting)
;        + (bio3 * bio3-weighting)
        + (bio4 * bio4-weighting)
;        + (bio5 * bio5-weighting)
;        + (bio6 * bio6-weighting)
;        + (bio7 * bio7-weighting)
;        + (bio8 * bio8-weighting)
;        + (bio9 * bio9-weighting)
;        + (bio10 * bio10-weighting)
;        + (bio11 * bio11-weighting)
;        + (bio13 * bio13-weighting)
;        + (bio14 * bio14-weighting)
;        + (bio15 * bio15-weighting)
;        + (bio16 * bio16-weighting)
;        + (bio17 * bio17-weighting)
;        + (bio18 * bio18-weighting)
;        + (bio19 * bio19-weighting)
      ) / num-var-used) + 1
    ]

    ifelse (elevation > 0)
    [ set pcolor scale-color red value 10 0 ]
    [ set pcolor blue ] ]

  ask patches [set current-value value]

  set-default-shape groups "person"
  ;set-default-size groups "1"
  ask patches with [ARCH = "Bismarcks" ][
          sprout-groups spawn-number
    ;set size 1
    ;set color black
    ;set archipelago [ARCH] of patch-here
  ]

  ask groups [
    set size 1
    set color black
    set archipelago [ARCH] of patch-here
  ]
  ;;set-default-shape groups "person"
  ;;create-groups number-of-groups [
  ;;  set size 1
  ;;  set color black
  ;;  move-to one-of patches with [ not shade-of? blue pcolor and pxcor < -70 and pycor > 28 and pycor < 51]
    ;setxy startx starty
  ;;  set archipelago [ARCH] of patch-here
    ;set heading -5 + random 10 ; generally head east
    ;set settled? false
    ;set age random (50 * 12) ; average human age is 25
  ;;]


end

to display-order
  clear-turtles
  reset-ticks
   ask patches
  [
    ifelse (elevation > 0)
    [ set pcolor scale-color green tstep2 0 45
      ;set pcolor palette:scale-gradient [[255 0 0] [0 255 0] [0 0 255]] tstep2 0 55
    ]
    [ set pcolor blue ]
  ]
end

to display-order2
  reset-ticks
  clear-turtles
   ask patches
  [
    ifelse (elevation > 0)
    [ set pcolor scale-color green tstep 0 45
      ;set pcolor palette:scale-gradient [[255 0 0] [0 255 0] [0 0 255]] tstep2 0 55
    ]
    [ set pcolor blue ]
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
248
10
1140
463
-1
-1
4.0
1
12
1
1
1
0
0
0
1
-110
110
-55
55
0
0
1
ticks
30.0

BUTTON
27
14
121
52
NIL
setup
NIL
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

BUTTON
28
67
153
100
display-arch
display-countries
NIL
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

SWITCH
28
180
151
213
label-islands
label-islands
0
1
-1000

BUTTON
29
144
143
177
Display-Islands
display-islands
NIL
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

BUTTON
31
259
197
292
NIL
display-value-in-patches
NIL
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

SLIDER
25
483
197
516
rain-weighting
rain-weighting
0
10
2.0
0.1
1
NIL
HORIZONTAL

SLIDER
25
521
197
554
area-weighting
area-weighting
0
10
0.0
0.1
1
NIL
HORIZONTAL

SLIDER
25
560
197
593
elevation-weighting
elevation-weighting
0
10
0.0
0.1
1
NIL
HORIZONTAL

SLIDER
27
348
199
381
distance-weighting
distance-weighting
0
0.5
0.007
0.001
1
NIL
HORIZONTAL

BUTTON
31
220
94
253
NIL
go
T
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

SLIDER
23
440
195
473
num-var-used
num-var-used
1
21
1.0
1
1
NIL
HORIZONTAL

SLIDER
24
599
196
632
bio1-weighting
bio1-weighting
0
10
0.0
0.1
1
NIL
HORIZONTAL

SLIDER
24
638
196
671
bio2-weighting
bio2-weighting
0
10
0.0
0.1
1
NIL
HORIZONTAL

SLIDER
24
676
196
709
bio3-weighting
bio3-weighting
0
10
0.0
0.1
1
NIL
HORIZONTAL

SLIDER
26
716
198
749
bio4-weighting
bio4-weighting
0
10
0.0
1
1
NIL
HORIZONTAL

SLIDER
270
497
442
530
bio5-weighting
bio5-weighting
0
10
0.0
1
1
NIL
HORIZONTAL

SLIDER
270
538
442
571
bio6-weighting
bio6-weighting
0
10
0.0
1
1
NIL
HORIZONTAL

SLIDER
271
583
443
616
bio7-weighting
bio7-weighting
0
10
0.0
1
1
NIL
HORIZONTAL

SLIDER
271
623
443
656
bio8-weighting
bio8-weighting
0
10
0.0
1
1
NIL
HORIZONTAL

SLIDER
270
665
442
698
bio9-weighting
bio9-weighting
0
10
4.0
1
1
NIL
HORIZONTAL

SLIDER
270
706
442
739
bio10-weighting
bio10-weighting
0
10
0.0
1
1
NIL
HORIZONTAL

SLIDER
460
500
632
533
bio11-weighting
bio11-weighting
0
10
0.0
1
1
NIL
HORIZONTAL

SLIDER
460
544
632
577
bio13-weighting
bio13-weighting
0
10
0.0
1
1
NIL
HORIZONTAL

SLIDER
460
586
632
619
bio14-weighting
bio14-weighting
0
10
0.0
1
1
NIL
HORIZONTAL

SLIDER
461
625
633
658
bio15-weighting
bio15-weighting
0
10
0.0
1
1
NIL
HORIZONTAL

SLIDER
461
668
633
701
bio16-weighting
bio16-weighting
0
10
1.0
1
1
NIL
HORIZONTAL

SLIDER
463
706
635
739
bio17-weighting
bio17-weighting
0
10
9.0
1
1
NIL
HORIZONTAL

SLIDER
653
501
825
534
bio18-weighting
bio18-weighting
0
10
0.0
1
1
NIL
HORIZONTAL

SLIDER
653
547
825
580
bio19-weighting
bio19-weighting
0
10
0.0
1
1
NIL
HORIZONTAL

PLOT
661
591
1138
808
Archipelago Group Populations 
Timesteps
Groups
0.0
40.0
0.0
400.0
false
true
"" ""
PENS
"Solomons" 1.0 0 -2674135 true "" "plot count turtles with [archipelago = \"Solomons\"]"
"Reef/Santa Cruz" 1.0 0 -7500403 true "" "plot count turtles with [archipelago = \"ReefSantaCruz\"]"
"Vanuatu" 1.0 0 -13840069 true "" "plot count turtles with [archipelago = \"Vanuatu\"]"
"New Caledonia" 1.0 0 -6459832 true "" "plot count turtles with [archipelago = \"New Caledonia\"]"
"Fiji" 1.0 0 -10649926 true "" "plot count turtles with [archipelago = \"Fiji\"]"
"Samoa" 1.0 0 -2064490 true "" "plot count turtles with [archipelago = \"Samoa\"]"
"Tonga" 1.0 0 -16777216 true "" "plot count turtles with [archipelago = \"Tonga\"]"

OUTPUT
271
748
511
896
11

TEXTBOX
28
770
178
798
setup - wait - select values - display value in patches - go.
11
0.0
1

SLIDER
29
394
201
427
max-trip-distance
max-trip-distance
0
300
300.0
1
1
NIL
HORIZONTAL

BUTTON
865
503
969
536
display-order
display-order
NIL
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

BUTTON
996
506
1106
539
display order2
display-order2
NIL
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

SWITCH
29
105
165
138
label-arch
label-arch
0
1
-1000

SLIDER
25
304
197
337
spawn-number
spawn-number
0
2
1.0
1
1
NIL
HORIZONTAL

SWITCH
101
221
217
254
RangeExpan
RangeExpan
1
1
-1000

@#$#@#$#@
## WHAT IS IT?

(a general understanding of what the model is trying to show or explain)

## HOW IT WORKS

(what rules the agents use to create the overall behavior of the model)

## HOW TO USE IT

(how to use the model, including a description of each of the items in the Interface tab)

## THINGS TO NOTICE

(suggested things for the user to notice while running the model)

## THINGS TO TRY

(suggested things for the user to try to do (move sliders, switches, etc.) with the model)

## EXTENDING THE MODEL

(suggested things to add or change in the Code tab to make the model more complicated, detailed, accurate, etc.)

## NETLOGO FEATURES

(interesting or unusual features of NetLogo that the model uses, particularly in the Code tab; or where workarounds were needed for missing features)

## RELATED MODELS

(models in the NetLogo Models Library and elsewhere which are of related interest)

## CREDITS AND REFERENCES

(a reference to the model's URL on the web if it has one, as well as any other necessary credits, citations, and links)
@#$#@#$#@
default
true
0
Polygon -7500403 true true 150 5 40 250 150 205 260 250

airplane
true
0
Polygon -7500403 true true 150 0 135 15 120 60 120 105 15 165 15 195 120 180 135 240 105 270 120 285 150 270 180 285 210 270 165 240 180 180 285 195 285 165 180 105 180 60 165 15

arrow
true
0
Polygon -7500403 true true 150 0 0 150 105 150 105 293 195 293 195 150 300 150

box
false
0
Polygon -7500403 true true 150 285 285 225 285 75 150 135
Polygon -7500403 true true 150 135 15 75 150 15 285 75
Polygon -7500403 true true 15 75 15 225 150 285 150 135
Line -16777216 false 150 285 150 135
Line -16777216 false 150 135 15 75
Line -16777216 false 150 135 285 75

bug
true
0
Circle -7500403 true true 96 182 108
Circle -7500403 true true 110 127 80
Circle -7500403 true true 110 75 80
Line -7500403 true 150 100 80 30
Line -7500403 true 150 100 220 30

butterfly
true
0
Polygon -7500403 true true 150 165 209 199 225 225 225 255 195 270 165 255 150 240
Polygon -7500403 true true 150 165 89 198 75 225 75 255 105 270 135 255 150 240
Polygon -7500403 true true 139 148 100 105 55 90 25 90 10 105 10 135 25 180 40 195 85 194 139 163
Polygon -7500403 true true 162 150 200 105 245 90 275 90 290 105 290 135 275 180 260 195 215 195 162 165
Polygon -16777216 true false 150 255 135 225 120 150 135 120 150 105 165 120 180 150 165 225
Circle -16777216 true false 135 90 30
Line -16777216 false 150 105 195 60
Line -16777216 false 150 105 105 60

car
false
0
Polygon -7500403 true true 300 180 279 164 261 144 240 135 226 132 213 106 203 84 185 63 159 50 135 50 75 60 0 150 0 165 0 225 300 225 300 180
Circle -16777216 true false 180 180 90
Circle -16777216 true false 30 180 90
Polygon -16777216 true false 162 80 132 78 134 135 209 135 194 105 189 96 180 89
Circle -7500403 true true 47 195 58
Circle -7500403 true true 195 195 58

circle
false
0
Circle -7500403 true true 0 0 300

circle 2
false
0
Circle -7500403 true true 0 0 300
Circle -16777216 true false 30 30 240

cow
false
0
Polygon -7500403 true true 200 193 197 249 179 249 177 196 166 187 140 189 93 191 78 179 72 211 49 209 48 181 37 149 25 120 25 89 45 72 103 84 179 75 198 76 252 64 272 81 293 103 285 121 255 121 242 118 224 167
Polygon -7500403 true true 73 210 86 251 62 249 48 208
Polygon -7500403 true true 25 114 16 195 9 204 23 213 25 200 39 123

cylinder
false
0
Circle -7500403 true true 0 0 300

dot
false
0
Circle -7500403 true true 90 90 120

face happy
false
0
Circle -7500403 true true 8 8 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Polygon -16777216 true false 150 255 90 239 62 213 47 191 67 179 90 203 109 218 150 225 192 218 210 203 227 181 251 194 236 217 212 240

face neutral
false
0
Circle -7500403 true true 8 7 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Rectangle -16777216 true false 60 195 240 225

face sad
false
0
Circle -7500403 true true 8 8 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Polygon -16777216 true false 150 168 90 184 62 210 47 232 67 244 90 220 109 205 150 198 192 205 210 220 227 242 251 229 236 206 212 183

fish
false
0
Polygon -1 true false 44 131 21 87 15 86 0 120 15 150 0 180 13 214 20 212 45 166
Polygon -1 true false 135 195 119 235 95 218 76 210 46 204 60 165
Polygon -1 true false 75 45 83 77 71 103 86 114 166 78 135 60
Polygon -7500403 true true 30 136 151 77 226 81 280 119 292 146 292 160 287 170 270 195 195 210 151 212 30 166
Circle -16777216 true false 215 106 30

flag
false
0
Rectangle -7500403 true true 60 15 75 300
Polygon -7500403 true true 90 150 270 90 90 30
Line -7500403 true 75 135 90 135
Line -7500403 true 75 45 90 45

flower
false
0
Polygon -10899396 true false 135 120 165 165 180 210 180 240 150 300 165 300 195 240 195 195 165 135
Circle -7500403 true true 85 132 38
Circle -7500403 true true 130 147 38
Circle -7500403 true true 192 85 38
Circle -7500403 true true 85 40 38
Circle -7500403 true true 177 40 38
Circle -7500403 true true 177 132 38
Circle -7500403 true true 70 85 38
Circle -7500403 true true 130 25 38
Circle -7500403 true true 96 51 108
Circle -16777216 true false 113 68 74
Polygon -10899396 true false 189 233 219 188 249 173 279 188 234 218
Polygon -10899396 true false 180 255 150 210 105 210 75 240 135 240

house
false
0
Rectangle -7500403 true true 45 120 255 285
Rectangle -16777216 true false 120 210 180 285
Polygon -7500403 true true 15 120 150 15 285 120
Line -16777216 false 30 120 270 120

leaf
false
0
Polygon -7500403 true true 150 210 135 195 120 210 60 210 30 195 60 180 60 165 15 135 30 120 15 105 40 104 45 90 60 90 90 105 105 120 120 120 105 60 120 60 135 30 150 15 165 30 180 60 195 60 180 120 195 120 210 105 240 90 255 90 263 104 285 105 270 120 285 135 240 165 240 180 270 195 240 210 180 210 165 195
Polygon -7500403 true true 135 195 135 240 120 255 105 255 105 285 135 285 165 240 165 195

line
true
0
Line -7500403 true 150 0 150 300

line half
true
0
Line -7500403 true 150 0 150 150

pentagon
false
0
Polygon -7500403 true true 150 15 15 120 60 285 240 285 285 120

person
false
0
Circle -7500403 true true 110 5 80
Polygon -7500403 true true 105 90 120 195 90 285 105 300 135 300 150 225 165 300 195 300 210 285 180 195 195 90
Rectangle -7500403 true true 127 79 172 94
Polygon -7500403 true true 195 90 240 150 225 180 165 105
Polygon -7500403 true true 105 90 60 150 75 180 135 105

plant
false
0
Rectangle -7500403 true true 135 90 165 300
Polygon -7500403 true true 135 255 90 210 45 195 75 255 135 285
Polygon -7500403 true true 165 255 210 210 255 195 225 255 165 285
Polygon -7500403 true true 135 180 90 135 45 120 75 180 135 210
Polygon -7500403 true true 165 180 165 210 225 180 255 120 210 135
Polygon -7500403 true true 135 105 90 60 45 45 75 105 135 135
Polygon -7500403 true true 165 105 165 135 225 105 255 45 210 60
Polygon -7500403 true true 135 90 120 45 150 15 180 45 165 90

sheep
false
15
Circle -1 true true 203 65 88
Circle -1 true true 70 65 162
Circle -1 true true 150 105 120
Polygon -7500403 true false 218 120 240 165 255 165 278 120
Circle -7500403 true false 214 72 67
Rectangle -1 true true 164 223 179 298
Polygon -1 true true 45 285 30 285 30 240 15 195 45 210
Circle -1 true true 3 83 150
Rectangle -1 true true 65 221 80 296
Polygon -1 true true 195 285 210 285 210 240 240 210 195 210
Polygon -7500403 true false 276 85 285 105 302 99 294 83
Polygon -7500403 true false 219 85 210 105 193 99 201 83

square
false
0
Rectangle -7500403 true true 30 30 270 270

square 2
false
0
Rectangle -7500403 true true 30 30 270 270
Rectangle -16777216 true false 60 60 240 240

star
false
0
Polygon -7500403 true true 151 1 185 108 298 108 207 175 242 282 151 216 59 282 94 175 3 108 116 108

target
false
0
Circle -7500403 true true 0 0 300
Circle -16777216 true false 30 30 240
Circle -7500403 true true 60 60 180
Circle -16777216 true false 90 90 120
Circle -7500403 true true 120 120 60

tree
false
0
Circle -7500403 true true 118 3 94
Rectangle -6459832 true false 120 195 180 300
Circle -7500403 true true 65 21 108
Circle -7500403 true true 116 41 127
Circle -7500403 true true 45 90 120
Circle -7500403 true true 104 74 152

triangle
false
0
Polygon -7500403 true true 150 30 15 255 285 255

triangle 2
false
0
Polygon -7500403 true true 150 30 15 255 285 255
Polygon -16777216 true false 151 99 225 223 75 224

truck
false
0
Rectangle -7500403 true true 4 45 195 187
Polygon -7500403 true true 296 193 296 150 259 134 244 104 208 104 207 194
Rectangle -1 true false 195 60 195 105
Polygon -16777216 true false 238 112 252 141 219 141 218 112
Circle -16777216 true false 234 174 42
Rectangle -7500403 true true 181 185 214 194
Circle -16777216 true false 144 174 42
Circle -16777216 true false 24 174 42
Circle -7500403 false true 24 174 42
Circle -7500403 false true 144 174 42
Circle -7500403 false true 234 174 42

turtle
true
0
Polygon -10899396 true false 215 204 240 233 246 254 228 266 215 252 193 210
Polygon -10899396 true false 195 90 225 75 245 75 260 89 269 108 261 124 240 105 225 105 210 105
Polygon -10899396 true false 105 90 75 75 55 75 40 89 31 108 39 124 60 105 75 105 90 105
Polygon -10899396 true false 132 85 134 64 107 51 108 17 150 2 192 18 192 52 169 65 172 87
Polygon -10899396 true false 85 204 60 233 54 254 72 266 85 252 107 210
Polygon -7500403 true true 119 75 179 75 209 101 224 135 220 225 175 261 128 261 81 224 74 135 88 99

wheel
false
0
Circle -7500403 true true 3 3 294
Circle -16777216 true false 30 30 240
Line -7500403 true 150 285 150 15
Line -7500403 true 15 150 285 150
Circle -7500403 true true 120 120 60
Line -7500403 true 216 40 79 269
Line -7500403 true 40 84 269 221
Line -7500403 true 40 216 269 79
Line -7500403 true 84 40 221 269

wolf
false
0
Polygon -16777216 true false 253 133 245 131 245 133
Polygon -7500403 true true 2 194 13 197 30 191 38 193 38 205 20 226 20 257 27 265 38 266 40 260 31 253 31 230 60 206 68 198 75 209 66 228 65 243 82 261 84 268 100 267 103 261 77 239 79 231 100 207 98 196 119 201 143 202 160 195 166 210 172 213 173 238 167 251 160 248 154 265 169 264 178 247 186 240 198 260 200 271 217 271 219 262 207 258 195 230 192 198 210 184 227 164 242 144 259 145 284 151 277 141 293 140 299 134 297 127 273 119 270 105
Polygon -7500403 true true -1 195 14 180 36 166 40 153 53 140 82 131 134 133 159 126 188 115 227 108 236 102 238 98 268 86 269 92 281 87 269 103 269 113

x
false
0
Polygon -7500403 true true 270 75 225 30 30 225 75 270
Polygon -7500403 true true 30 75 75 30 270 225 225 270
@#$#@#$#@
NetLogo 6.0.3
@#$#@#$#@
@#$#@#$#@
@#$#@#$#@
@#$#@#$#@
@#$#@#$#@
default
0.0
-0.2 0 0.0 1.0
0.0 1 1.0 0.0
0.2 0 0.0 1.0
link direction
true
0
Line -7500403 true 150 150 90 180
Line -7500403 true 150 150 210 180
@#$#@#$#@
0
@#$#@#$#@
